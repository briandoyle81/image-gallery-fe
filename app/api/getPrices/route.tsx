...
import { estimateContractCallCosts, ChainCost } from '@/app/util/EstimateTxCosts';
import { contractData } from '@/app/contracts/contractData';
import { mainnet, avalanche, base, bsc, arbitrum, polygon, optimism, flowMainnet } from 'wagmi/chains';
...
export async function POST(request: NextRequest) {
  const { baseImageGallery, bnbImageGallery, flowImageGallery, mainnetImageGallery, polygonImageGallery, optimismImageGallery, arbitrumImageGallery, avalancheImageGallery } = contractData;

  try {
    const { base64ImageStringLength } = await request.json();

    if (!base64ImageStringLength || typeof base64ImageStringLength !== 'number') {
    ...
    costs.push(await estimateContractCallCosts({
      chain: mainnet,
